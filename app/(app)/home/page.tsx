...
import { centsToDollars, getDateRangeFromPreset, DateRangePreset } from "@/components/utils";
...
export default function HomePage() {
  ...
  const [homePreset, setHomePreset] = useState<DateRangePreset>(() => {
    if (typeof window === "undefined") return "month";
    try {
      const stored = localStorage.getItem("tallyup.homeTimeRange");
      if (stored) {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem("tallyup.homeTimeRange", JSON.stringify({ preset: homePreset }));
    } catch {}
  }, [homePreset]);
  const { startDate, endDate, label } = useMemo(() => getDateRangeFromPreset(homePreset), [homePreset]);
